...
const ResTable* AssetManager::createResTable(Vector<Asset*>& assets) const
{
    ResTable* rt = mResources;
    if (rt) {
        LOG_FATAL("Ressource table has already been created!");
	return rt;
    }

    AutoMutex _l(mLock);

    ResTable* sharedRes = new ResTable();
    sharedRes->add(assets[0], (void*)(1), false, NULL);
    sharedRes = const_cast<AssetManager*>(this)->
	mZipSet.setZipResourceTable(String8("root"), sharedRes);
