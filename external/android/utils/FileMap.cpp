...
FileMap::FileMap(void)
    : mRefCount(1), mFileName(NULL), mBasePtr(NULL), mBaseLength(0),
      mDataPtr(NULL), mDataLength(0), mAnonymous(false)
{
}
 ...
 * XOBOTOS ADDITIONS
 */
FileMap::FileMap(void* ptr, size_t length)
    : mRefCount(1), mFileName(NULL), mBasePtr(ptr), mBaseLength(length),
      mDataPtr(ptr), mDataLength(length), mAnonymous(true)
{ }

FileMap*
FileMap::createSubMap(off_t offset, size_t size)
{
    assert(mDataPtr);
    assert(offset+size <= mDataLength);

    FileMap* map = new FileMap();
    map->mBasePtr = mBasePtr;
    map->mBaseLength = mBaseLength;
    map->mDataPtr = (char*)mDataPtr+offset;
    map->mDataLength = size;
    map->mAnonymous = true;
    mRefCount++;
    return map;
}
