    ...
    const sp<AaptSymbols>& symbols, int indent, bool includePrivate)
{
    ...
    fprintf(fp, "%spublic static class styleable {\n", indentStr);
    indent++;
...
static status_t writeSymbolClass(
    ...
    bool nonConstantId)
{
    fprintf(fp, "%spublic static class @%s {\n",
            getIndentSpace(indent), className.string());
    indent++;
    ...
    for (i=0; i<N; i++) {
        ...
        fprintf(fp, "%spublic const string %s=\"%s\";\n",
                getIndentSpace(indent),
