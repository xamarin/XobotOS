    ...
    ZipEntry** ppEntry)
{
    ...
    /*
     * FIXME:
     * Don't use the .zip file's modification time since it's different
     * each time we run and we don't want to create a different output when
     * none of the input files have changed.
     */
    modWhen = inputFp ? getModTime(fileno(inputFp)) : -1;
